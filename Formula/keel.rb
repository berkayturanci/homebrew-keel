class Keel < Formula
  ...
  url "https://github.com/berkayturanci/keel/archive/refs/tags/v1.20.0.tar.gz"
  sha256 "24114a278eb629ff9004e67445822403f1fff72d12b8702b8f7b1fc4c4a4ba51"
  license "Apache-2.0"
  ...
  #
  # These two values are pinned, not rendered, and that is correct: PyYAML 6.0.3
  # is already published, so its url and digest are knowable at commit time and
  # are checked offline by `tests/test_release_docs.py` and online by the tap's
  # own suite. The pair that could not be knowable at commit time is the one
  # above, which is why only that pair is a placeholder.
  resource "PyYAML" do
  ...
  test do
    assert_match "keel 1.20.0", shell_output("#{bin}/keel version")
  end
end
